...
from ticket.models import Comment, Task
from ticket.utils import send_email_to_thread_participants, send_notify_mail
...
def test_defaults(settings):
    """ticket's `defaults` module provides reasonable default values for unspecified settings.
    If a value is NOT set, it should be pulled from the hash in defaults.py.
    If a value IS set, it should be respected.
    n.b. TICKET_STAFF_ONLY which defaults to True in the `defaults` module."""

    key = "TICKET_STAFF_ONLY"
