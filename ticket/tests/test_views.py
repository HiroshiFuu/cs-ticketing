...
from ticket.models import Task, TaskList
...
def test_view_list_lists(todo_setup, admin_client):
    url = reverse("ticket:lists")
    response = admin_client.get(url)
    assert response.status_code == 200


def test_view_reorder(todo_setup, admin_client):
    url = reverse("ticket:reorder_tasks")
    response = admin_client.get(url)
...
def test_view_external_add(todo_setup, admin_client, settings):
    ...
    settings.TICKET_DEFAULT_LIST_SLUG = default_list.slug
    assert settings.TICKET_DEFAULT_LIST_SLUG == default_list.slug
    url = reverse("ticket:external_add")
    response = admin_client.get(url)
    assert response.status_code == 200


def test_view_mine(todo_setup, admin_client):
    url = reverse("ticket:mine")
    response = admin_client.get(url)
    assert response.status_code == 200
...
def test_view_list_completed(todo_setup, admin_client):
    ...
    url = reverse(
        "ticket:list_detail_completed", kwargs={"list_id": tlist.id, "list_slug": tlist.slug}
    )
    response = admin_client.get(url)
    assert response.status_code == 200
...
def test_view_list(todo_setup, admin_client):
    tlist = TaskList.objects.get(slug="zip")
    url = reverse("ticket:list_detail", kwargs={"list_id": tlist.id, "list_slug": tlist.slug})
    response = admin_client.get(url)
    assert response.status_code == 200


def test_view_add_list(todo_setup, admin_client):
    url = reverse("ticket:add_list")
    response = admin_client.get(url)
    assert response.status_code == 200
...
def test_view_task_detail(todo_setup, admin_client):
    ...
    url = reverse("ticket:task_detail", kwargs={"task_id": task.id})
    response = admin_client.get(url)
    assert response.status_code == 200
...
def test_del_task(todo_setup, admin_user, client):
    task = Task.objects.first()
    url = reverse("ticket:delete_task", kwargs={"task_id": task.id})
...
def test_task_toggle_done(todo_setup, admin_user, client):
    ...
    url = reverse("ticket:task_toggle_done", kwargs={"task_id": task.id})
    # View accepts POST, not GET
    client.login(username="admin", password="password")
    response = client.get(url)
    assert response.status_code == 403
...
def test_no_javascript_in_task_note(todo_setup, client):
    ...
    url = reverse("ticket:list_detail", kwargs={"list_id": task_list.id, "list_slug": task_list.slug})
...
def test_created_by_unchanged(todo_setup, client):
    ...
    url_add_task = reverse(
        "ticket:list_detail", kwargs={"list_id": task_list.id, "list_slug": task_list.slug}
    )
    ...
    url_edit_task = reverse("ticket:task_detail", kwargs={"task_id": task.id})
...
def test_view_del_list_nonadmin(todo_setup, client):
    tlist = TaskList.objects.get(slug="zip")
    url = reverse("ticket:del_list", kwargs={"list_id": tlist.id, "list_slug": tlist.slug})
    client.login(username="you", password="password")
    response = client.get(url)
...
def test_setting_TICKET_STAFF_ONLY_False(todo_setup, client, settings):
    ...
    settings.TICKET_STAFF_ONLY = False
    url = reverse("ticket:lists")
    client.login(username="u2", password="password")
    response = client.get(url)
    assert response.status_code == 200
...
def test_setting_TICKET_STAFF_ONLY_True(todo_setup, client, settings, django_user_model):
    # We use Django's user_passes_test to call `staff_check` utility function on some views.
    # Just testing one view here...
    settings.TICKET_STAFF_ONLY = True
    url = reverse("ticket:lists")
