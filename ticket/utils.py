...
def staff_check(user):
    """If TICKET_STAFF_ONLY is set to True, limit view access to staff users only.
        # FIXME: More granular access control needed - see
        https://github.com/shacker/cs-support-ticket/issues/50
    """

    if defaults("TICKET_STAFF_ONLY"):
...
def user_can_read_task(task, user):
    return task.task_list == user.company or user.is_staff


def todo_get_backend(task):
    """Returns a mail backend for some task"""
    mail_backends = getattr(settings, "TICKET_MAIL_BACKENDS", None)
...
def todo_send_mail(user, task, subject, body, recip_list):
    ...
    thread_message_id = "<thread-{}@cs-support-ticket>".format(task.pk)
...
def send_notify_mail(new_task):
    ...
    subject = render_to_string("ticket/email/assigned_subject.txt", {"task": new_task})
    body = render_to_string(
        "ticket/email/assigned_body.txt", {"task": new_task, "site": current_site}
    )
...
def send_email_to_thread_participants(task, msg_body, user, subject=None):
    ...
    if not subject:
        subject = render_to_string("ticket/email/assigned_subject.txt", {"task": task})

    email_body = render_to_string(
        "ticket/email/newcomment_body.txt",
...
def toggle_task_completed(task_id: int, credits=None) -> bool:
    """Toggle the `completed` bool on Task from True to False or vice versa."""
    try:
        task = Task.objects.get(id=task_id)
        task.completed = not task.completed
        if not task.task_list.subscription_based:
            if task.completed:
                task.task_list.credits -= credits
                task.credits_comsumed = credits
            else:
                task.task_list.credits += task.credits_comsumed
                task.credits_comsumed = None
            task.task_list.save()
        task.save()
        return True
