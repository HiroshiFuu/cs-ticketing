...
from ticket.models import Comment, Task, TaskList
...
DJANGO_TICKET_THREAD = re.compile(r"<thread-(\d+)@cs-support-ticket>")
...
def parse_references(task_list, references):
    ...
    for related_message in references.split():
        ...
        match = re.match(DJANGO_TICKET_THREAD, related_message)
        if match is None:
...
def insert_message(task_list, message, priority, task_title_format):
    ...
    # (generated by cs-support-ticket) could be found, use it
